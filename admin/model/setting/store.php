...
class ModelSettingStore extends Model {
    ...
    public function getEmp($store_id){
        $query = $this->db->query('select * from '.DB_PREFIX.'emp_to_store where store_id='.$store_id);

        return $query->rows;
    }
    public function addNewEmp($data){
        $sql = 'insert into '.DB_PREFIX.'emp_to_store set name="'.$this->db->escape($data['name']).'",post="'.$this->db->escape($data['post']).'",phone="'.$this->db->escape($data['phone']).'",store_id='.$data['store_id'];
        $this->db->query($sql );
        return true;
    }
}
