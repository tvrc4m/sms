...
<div class="contents">
    ...
    <div class="row-fluid">
        <div class="pull-right">
            <a onclick="addClient()" class="btn">添加客户</a>
            <a onclick="addGroup()" class="btn">创建群组</a>
        </div>
    </div>
    <div class="separator">
        <span></span>
...
<script type="text/javascript">
    function delChannel(that){
        if(window.confirm('是否确认要删除?')){
            $.ajax({
                url:$(that).attr('href'),
                data:{channel:$(that).attr('channel')},
                dataType:'json',
                type:'GET',
    ...
    function editGroup(that){
        ...
        $.ajax({
            url:'index.php?route=client/ajax/getGroup&token=<?php echo $this->session->data["token"]; ?>',
    ...
    function delGroup(that){
        ...
        if(window.confirm('是否确认要删除?')){
            ...
            $.ajax({
                url:'index.php?route=client/ajax/delGroup&token=<?php echo $this->session->data["token"]; ?>',
    ...
    function editClient(that,client_id){
        $.ajax({
            url:'index.php?route=client/ajax/getClient&token=<?php echo $this->session->data["token"]; ?>',
    ...
    function updateClient(){
        ...
        $.ajax({
            url:'index.php?route=client/ajax/updateClient&token=<?php echo $this->session->data["token"]; ?>',
    ...
    function updateClientGroup(){
        ...
        $.ajax({
            url:'index.php?route=client/ajax/updateGroup&token=<?php echo $this->session->data["token"]; ?>',
...
<script type="text/javascript">
    ...
    var currentUrl = 'index.php?route=client/client/getList&token=<?php echo $token; ?>';
    ...
    function loadListInfo(url) {
        ...
        $.ajax({
            ...
            success: function(html) {
                ...
                var minus = timestampend - timestampstart;
                if(minus < 1000) {
                    setTimeout(function(){
    ...
    loadListInfo(currentUrl);

    $(function(){

        $(".service-tab a").click(function(){
