...
class ControllerCommonHome extends Controller {   
	public function index() {
		...
		$this->data['text_latest_100_orders'] = $this->language->get('text_latest_100_orders');
    	...
    	/*foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('sale/order/info', 't=' . $this->session->data['t'] . '&order_id=' . $result['order_id'], 'SSL')
			);
	...
	public function login() {
		...
		if (isset($this->request->get['route'])) {
			...
			if ($this->config->get('config_t_ignore')) {
				$config_ignore = unserialize($this->config->get('config_t_ignore'));
			}
				
			$ignore = array_merge($ignore, $config_ignore);
			if (!in_array($route, $ignore) && (!isset($this->request->get['t']) || !isset($this->session->data['t']) || ($this->request->get['t'] != $this->session->data['t']))) {
				return $this->forward('common/login');
			}
		} else {
			if (!isset($this->request->get['t']) || !isset($this->session->data['t']) || ($this->request->get['t'] != $this->session->data['t'])) {
				return $this->forward('common/login');
			}
		}
	}
	...
	public function permission() {
		if (isset($this->request->get['route'])) {
			...
			if (!in_array($route, $ignore) && !$this->customer->hasPermission('access', $route)) {
