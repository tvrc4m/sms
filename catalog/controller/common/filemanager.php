...
class ControllerCommonFileManager extends Controller {
	...
	public function create() {
		...
		if (!$this->customer->hasPermission('modify', 'common/filemanager')) {
    ...
    public function muti_upload() {
        $this->language->load('common/filemanager');

        $json = array();
        if(!empty($this->request->post['directory'])){
            $image_dir = 'data/' .$this->request->post['directory'];
        }else{
            $image_dir = 'data/' . date('Y/m/d/');
        }
        $directory = DIR_IMAGE . $image_dir;
        if (isset($this->request->files['image']) && $this->request->files['image']['tmp_name']) {

            $filename = basename(html_entity_decode($this->request->files['image']['name'], ENT_QUOTES, 'UTF-8'));
            $extensionName = strtolower(strrchr($filename, '.'));
            ...
            if (!file_exists($directory)) {
                if (!mkdir($directory, 0755, true)) {
                    $json['error'] = 'Create directory failed';
                }
            }
            $MB = 1024*1024;
            $minSize = 1;
            $maxSize = 1024*1024;
            if ($this->request->files['image']['size'] > $maxSize || $this->request->files['image']['size'] <= $minSize) {
                $json['error'] = sprintf($this->language->get('error_file_size'),$minSize." KB",(intval($maxSize)/$MB)." MB ");
            }
        ...
        list($width, $height) = getimagesize($this->request->files['image']['tmp_name']);
        if (file_exists($directory . '/' . $filename)) {
            $filename = time() . '-' . $width . 'x' . $height .  '.' . $filename;
        }else{
            $fileNamePiece = explode('.', $filename);
            $type = $fileNamePiece[count($fileNamePiece) - 1];
            array_pop($fileNamePiece);
            $name = implode('.',$fileNamePiece);
        ...
        if (!isset($json['error'])) {
            if (@move_uploaded_file($this->request->files['image']['tmp_name'], $directory . '/' . $filename)) {
                $json['success'] = $this->language->get('text_uploaded');
                $this->load->model('tool/image');
                $json['image'] = $image_dir . $filename;

                if(isset($this->request->get['tag'])) {
                    $json['thumb'] = $this->model_tool_image->resize($json['image'], 200, 200);
                }else{
                    $json['thumb'] = $this->model_tool_image->resize($json['image'], 100, 100);
                }
            } else {
                $json['error'] = $this->language->get('error_uploaded');
            }
